...
typedef struct __black_scholes_args_parallel_t {
  /**
   * [IN] Various parameters of the Black-Scholes MC method.  
   */
  double S, E, r, sigma, T;
  ...
  /**
   * The amount of iterations for a particular thread.
   */
  int quota;
  
} black_scholes_args_parallel_t;
...
void test_num_gen_func(int nThreads, int amount);

void *createNumbers(void* args);

typedef struct __test_num_gen_t {
    int index;
    long amount;
} test_num_gen_t;
/**
...
black_scholes (confidence_interval_t* interval,
	       ...
	       const int M);
void
black_scholes_parallel (confidence_interval_t* interval,
	       const double S,
	       const double E,
	       const double r,
	       const double sigma,
